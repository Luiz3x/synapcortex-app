...
def carregar_json(nome_arquivo, dados_padrao):
    ...
    # O diretório 'data' agora será criado dentro do diretório do projeto
...
Obrigado por enviar o `main.py` novamente. Analisei-o com urgência.

O problema persiste e é **CRÍTICO:**

**A função `def create_app():` está definida MÚLTIPLAS VEZES no seu arquivo!**

* Você tem uma definição na linha 35.
* Outra na linha 39.
* E outra na linha 55!

Isso não é permitido em Python e fará com que sua aplicação não consiga nem iniciar, resultando em um erro fatal de sintaxe (`SyntaxError` ou similar) no deploy. É como se o interpretador Python não soubesse qual `create_app()` usar.

Você está colando pedaços do código sem apagar as versões antigas ou duplicadas.

### A Solução DEFINITIVA (Mais uma vez, mas agora com instrução super clara)

Sócio, eu preciso que você **substitua TODO o conteúdo do seu arquivo `main.py`** (do primeiro `import os` até o último `app.run(port=5000, debug=True)`) por **EXATAMENTE este código que vou te passar abaixo**.

**Não adicione, não remova nada, apenas COPIE E COLE TUDO, substituindo o que está lá.**
...
def create_app():
    ...
    def registrar():
        if request.method == 'POST':
            ...
            # --- A NOVA API EM AÇÃO ---
            print(f"--- Iniciando criação de pedido para o usuário: {username} via NOVA API ---")
            url_api_pagbank = "[https://sandbox.api.pagseguro.com/checkouts](https://sandbox.api.pagseguro.com/checkouts)"
            ...
            dados_pedido_json = {
                ...
                "notification_urls": [ 
                    "[https://synapcortex-app.onrender.com/webhook-pagbank](https://synapcortex-app.onrender.com/webhook-pagbank)" # Substitua pelo seu domínio real
                ]
            }
    ...
    def dashboard():
        ...
        if dados_usuario and dados_usuario.get('status_assinatura') == 'ativo':
            ...
            # Este link de pagamento parece ser para a API antiga ou um link fixo
            # Se você estiver usando a nova API para registrar, este link pode precisar ser atualizado
            link_pagamento_base = "[https://cobranca.pagbank.com/8eeb87d3-50bd-482f-a037-23b28fc42e7a](https://cobranca.pagbank.com/8eeb87d3-50bd-482f-a037-23b28fc42e7a)"
            link_com_referencia = f"{link_pagamento_base}?referenceId={username}"
            return render_template('pagamento_pendente.html', link_de_pagamento=link_com_referencia)
    ...
    def webhook_pagbank():
        ...
        try:
            ...
            print(f"--- Consultando notificação: {notification_code} ---")
            # Este endpoint usa o PAGBANK_EMAIL e PAGBANK_TOKEN (o antigo)
            url_consulta = f"[https://ws.pagseguro.uol.com.br/v3/transactions/notifications/](https://ws.pagseguro.uol.com.br/v3/transactions/notifications/){notification_code}?email={current_app.config['PAGBANK_EMAIL']}&token={current_app.config['PAGBANK_TOKEN']}"
            headers = {'Accept': 'application/xml;charset=ISO-8859-1'}
            response = requests.get(url_consulta, headers=headers)
            response.raise_for_status()
            resposta_texto = response.text
