# main.py - Versão 3.1 (Consolidada e Definitiva)
# Contém a lógica completa da Sala de Comando e todas as validações.
...
def dashboard():
    ...
    if status_assinatura == 'ativo':
        ...
        if data_fim_str:
            ...
            if hoje > data_fim:
                ...
                salvar_json(CAMINHO_USUARIOS, usuarios); status_assinatura = 'pendente'
            else:
    ...
    labels_grafico, dados_visualizacoes, dados_cliques = [], [], [] # Lógica do gráfico real a ser implementada aqui
...
def get_config():
    # Nota: Esta é uma configuração pública. No futuro, podemos torná-la específica do usuário.
    usuarios = carregar_json(CAMINHO_USUARIOS)
    # Por simplicidade, vamos pegar a config do primeiro usuário como padrão
    # A lógica ideal dependeria de como identificamos o site do cliente
    if usuarios:
        primeiro_email = next(iter(usuarios))
        config = usuarios[primeiro_email].get('configuracoes', {})
        return jsonify({
            'titulo': config.get('popup_titulo', 'Oferta Especial'),
            'mensagem': config.get('popup_mensagem', 'Temos algo para você!')
        })
    return jsonify({'titulo': 'Oferta', 'mensagem': 'Aproveite!'})
